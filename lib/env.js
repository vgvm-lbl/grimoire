...
const config = {
  ...
  // Grimoire server address (remote mode) — e.g. http://aid:3666
  ...
  port:     parseInt(process.env.GRIMOIRE_PORT || '3666', 10),
}
...
function requireMode(required = 'any') {
  ...
  if (required === 'remote' && !isRemote) {
    ...
    console.error('Set GRIMOIRE_HOST=http://aid:3666 in .env, then run: grim serve')
    process.exit(1)
  }
