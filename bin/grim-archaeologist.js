...
function buildFilePrompt(rel, content, lines) {
  return `Analyze this file. Write 2-3 paragraphs covering what it is, what it does, and any non-obvious design decisions or quirks. Use bullet lists for key components, exports, or notable items where that's clearer than prose.
...
function buildImagePrompt(rel) {
  return `Describe this image concisely. Cover: what it depicts, any text or labels visible, apparent purpose or context, and anything notable about style or content. 2-3 paragraphs.\n\nFile: ${rel}`
}
...
async function runFileDig(filePath, opts = {}) {
  const abs  = path.resolve(filePath)
  const rel  = path.basename(abs)
  const ext  = path.extname(rel).toLowerCase()
  const out  = archDir(filePath)
  ensureDir(out)
