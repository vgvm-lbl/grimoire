...
async function runSynthesis(projectDir, opts = {}) {
  ...
  const final   = await ask({ prompt, system: ARCH_SYSTEM, task: 'dreaming', timeout: 300000 })
...
async function main() {
  ...
  if (args.dig || args.overview || args.files || args.synth) {
    ...
    if (args.synth)    return runSynthesis(target, opts)
  }
...
module.exports = { runDig, runOverview, runFilePass, runSynthesis, analyzeProject, findProjects, detectLanguages, detectEra }
